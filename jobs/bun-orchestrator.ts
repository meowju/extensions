...
import { runLeanAgent } from "../../kernel/src/core/lean-agent.ts";
...
import { searchMemory, formatSearchResults, storeMemory } from "../../kernel/src/sidecars/memory-fts.ts";
import { GovernanceEngine } from "../src/sidecars/governance-engine.ts";

const __dirname = dirname(fileURLToPath(import.meta.url));

// Manual .env loader for Bun - find the closest .env searching upwards
function findEnv(startDir: string): string | null {
  let current = startDir;
  while (current !== dirname(current)) {
    const p = join(current, ".env");
...
class Orchestrator {
  ...
  constructor() {
    ...
    const maxConcurrent = parseInt(process.env.MEOW_MAX_CONCURRENCY || "10");
    const tpsBudget = parseInt(process.env.MEOW_TPS_BUDGET || "30");
    this.throttler = new Throttler(maxConcurrent, tpsBudget);
    ...
    if (existsSync(evolveEpochDir)) {
      ...
      if (epochs.length > 0) {
        ...
        if (existsSync(validationDir)) {
          ...
          const sortedValidations = validations.sort().reverse();
...
Recent Learnings: ${recentLearnings}${errorContext}`;
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        const runJobs = new Set<string>((decisions as any[]).filter((d: any) => d.type === "RUN").map((d: any) => String(d.job)));

        for (const job of this.jobs) {
          const namePrefix = job.name.split(":")[0];
          const isInRunList = runJobs.has(job.name) ||
            (namePrefix !== undefined && runJobs.has(namePrefix)) || // "EVOLVE" matches "EVOLVE: Research..."
            Array.from(runJobs).some((r: string) => job.name.toLowerCase().includes(r.toLowerCase()));
          if (!isInRunList) {
        ...
        const stats = this.throttler.getStats();
        console.log(`[orchestrator] Throttle Status: ${stats.active}/${this.throttler.maxConcurrent} active, ${stats.tokens.toFixed(1)} TPS budget`);
          ...
          if (action.type === "RUN") {
            ...
            if (job && job.status !== "running") {
              if (this.throttler.canRun()) {
                ...
                const stats = this.throttler.getStats();
                console.log(`[orchestrator] 🛑 Throttle active: Skipping start for ${job.name} (${stats.active} active, ${stats.tokens.toFixed(1)} tokens)`);
              }
            } else if (!job) {
...
/**
 * Throttler - limits concurrency and start rate (TPS)
 */
class Throttler {
  ...
  constructor(public maxConcurrent: number, tps: number) {
    this.maxTokens = tps;
    this.refillRate = tps;
    this.tokens = tps;
  ...
  canRun(): boolean {
    this.refill();
    return this.activeCount < this.maxConcurrent && this.tokens >= 1;
  }
