...
export interface Tool {
  ...
  parameters: Record<string, unknown>;
  execute(args: unknown, context: ToolContext): Promise<ToolResult>;
  /** Optional: Validate input before execution. Return error string if invalid. */
  validateInput?: (args: unknown) => string | null;
  /** Optional: Render a compact result summary for history/UI. */
  renderResult?: (result: ToolResult) => string;
  /** Optional: Cleanup function called when tool is aborted. */
  onInterrupt?: () => void;
  /** Optional: Maximum result size in characters before truncation. Default: unlimited. */
  maxResultSizeChars?: number;
  /** Optional: Interrupt behavior - 'cancel' (can be cancelled) or 'block' (runs to completion). Default: 'cancel'. */
  interruptBehavior?: () => "cancel" | "block";
  /** Optional: Get a compact summary of the tool call for display in history. */
...
// EPOCH 22: Dangerous path patterns that should be blocked even within workspace
...
// EPOCH 22: Path traversal detection
function hasPathTraversal(path: string): boolean {
  const normalized = path.replace(/\\/g, '/');
  return normalized.includes('../') || normalized.includes('..\\');
}

// EPOCH 22: Validate tool input before execution
function validateToolInput(toolName: string, args: unknown): { valid: boolean; error?: string } {
  ...
  switch (toolName) {
    ...
    case "edit": {
      ...
      if (hasPathTraversal(path)) {
        return { valid: false, error: `Path traversal detected: ${path}` };
      }
      if (isDangerousPath(path)) {
        return { valid: false, error: `Access denied: dangerous path ${path}` };
      }
      return { valid: true };
    }
    case "shell": {
      const cmd = argsObj.cmd as string;
      if (!cmd || typeof cmd !== "string") {
        return { valid: false, error: `Invalid command: ${cmd}` };
      }
      if (cmd.includes('..')) {
        // Check for path traversal in shell commands
        const cmdParts = cmd.split(' ');
        for (const part of cmdParts) {
          if (hasPathTraversal(part)) {
            return { valid: false, error: `Path traversal in shell command: ${part}` };
          }
        }
      }
      return { valid: true };
...
import { AgentState } from "../types/agent-state.ts";
...
const builtInTools: Tool[] = [
  {
    ...
    getToolUseSummary: (args: unknown) => {
      const { path } = args as { path: string };
  ...
  },
  {
    ...
    execute: async (args: unknown, context: ToolContext) => {
      ...
      // EPOCH 22: Shell hardening — reject dangerous commands
      const dangerousPatterns = [
        // Destructive: wipe disk/system
        /^\s*rm\s+-rf\s+\/(?:\S*)?$/,           // rm -rf / or rm -rf /path
        /^\s*>+\s*\/dev\/sd/,                    // > /dev/sda
        /^\s*dd\s+if=/,                          // dd if= (disk clone)
        /^\s*mkfs\./,                            // mkfs.* (format)
        /^\s*mount\s+--bind/,                    // mount --bind (privilege escalation)
        /^:\s*\(\)\s*;.*;$/,                     // :();:; (fork bomb pattern)
        // Privilege escalation
        /^.*sudo\s+su/,                          // sudo su
        /^.*chmod\s+777\s+\//,                   // chmod 777 /
        /^.*chmod\s+-R\s+777/,                   // chmod -R 777 (recursive)
        /^.*setenforce\s+0/,                      // disable SELinux
        /^.*iptables\s+-F/,                      // flush iptables
        // Data destruction
        /^.*shred\s+/,                           // shred (secure delete)
        /^.*nc\s+-e\s+\//,                       // nc -e /bin/sh (reverse shell)
        /^.*\/>\s*\/etc\/passwd/,                // overwrite passwd
        /^.*\/>\s*\/etc\/shadow/,               // overwrite shadow
      ];
      for (const pattern of dangerousPatterns) {
        if (pattern.test(cmd.trim())) {
          return { content: "", error: `[shell:DENIED] Dangerous command blocked: ${cmd}` };
        }
      }
  ...
  },
  {
    ...
    getToolUseSummary: (args: unknown) => {
      const { cmd } = args as { cmd: string };
      // Extract git subcommand (status, diff, commit, etc.)
      const parts = cmd.trim().split(/\s+/);
      const subCmd = parts[0] || cmd;
      return `git ${subCmd}`;
    },
  },
  {
...
/**
 * Get a compact summary of a tool call for display in history/UI.
 * Returns null if the tool doesn't have a summary function.
 */
export function getToolUseSummary(toolName: string, input: unknown): string | null {
  const tool = getTool(toolName);
  return tool?.getToolUseSummary?.(input) ?? null;
}
...
export async function executeTool(
  ...
  context.onStateChange?.(AgentState.EXECUTING, `Running ${toolName}...`);

  // EPOCH 22: Check if already aborted before starting
  context.abortSignal?.throwIfAborted?.();

  // EPOCH 22: Validate input before execution
  const validation = validateToolInput(toolName, args);
  if (!validation.valid) {
    return { content: "", error: `[${toolName}:INVALID] ${validation.error}` };
  }
  ...
  if (permission.action === "ask") {
    ...
    if (context.dangerous) {
      ...
      context.onStateChange?.(AgentState.WAITING_PERMISSION, "Waiting for permission...");
  ...
  // EPOCH 22: Timeout enforcement via Promise.race
  let timeoutId: ReturnType<typeof setTimeout> | null = null;
  let timeoutTimer: Promise<ToolResult> | null = null;

  if (context.timeoutMs && context.timeoutMs > 0) {
    timeoutTimer = new Promise<ToolResult>((resolve) => {
      timeoutId = setTimeout(() => {
        resolve({
          content: "",
          error: `[${toolName}:TIMEOUT] Operation timed out after ${context.timeoutMs}ms`,
        });
      }, context.timeoutMs);
    });
  }

  // EPOCH 22: Wrap with abort signal check
  const executeWithAbort = async (): Promise<ToolResult> => {
  ...
  try {
    if (timeoutTimer) {
      result = await Promise.race([executeWithAbort(), timeoutTimer]);
    } else {
      result = await executeWithAbort();
    }
  } finally {
    if (timeoutId) clearTimeout(timeoutId);
  }

  // EPOCH 17: Emit summarizing state when tool execution completes
  context.onStateChange?.(AgentState.SUMMARIZING, "Processing result...");
