...
import { AgentState } from "../src/types/agent-state.ts";
...
async function withSpinner<T>(
  promise: Promise<T>,
  initialMessage: string = "thinking...",
  onAbort?: () => void,
  ...
  const spin = async () => {
    while (!interrupted && isThinking) {
      process.stdout.write(`${colors.dim}${spinnerFrames[frame % spinnerFrames.length]} ${currentMessage}${colors.reset}\r`);
      frame++;
  ...
  // Wire up state change handler to update spinner message
  const stateHandler = (state: AgentState, message?: string) => {
    if (state === AgentState.THINKING) {
      currentMessage = message || t("thinking");
    } else if (state === AgentState.EXECUTING) {
      currentMessage = message || "Executing...";
    } else if (state === AgentState.WAITING_PERMISSION) {
      currentMessage = message || "Waiting for permission...";
    } else if (state === AgentState.SUMMARIZING) {
      currentMessage = message || "Summarizing...";
    } else if (state === AgentState.INDEXING) {
      currentMessage = message || "Indexing...";
    } else if (state === AgentState.READING) {
      currentMessage = message || "Reading...";
    } else if (state === AgentState.WRITING) {
      currentMessage = message || "Writing...";
    }
...
async function main() {
  ...
  const runAgent = async (prompt: string, options: LeanAgentOptions = {}) => {
    ...
    try {
      ...
      // Create a state change handler that updates spinner and forwards to existing callback
      const stateHandler = (state: AgentState, message?: string) => {
        options.onStateChange?.(state, message);
      };

      const result = await withSpinner(
        runLeanAgent(prompt, { dangerous, ...options, abortSignal: abortController?.signal, messages: agentMessages, onStateChange: stateHandler }),
        "thinking...",
        () => {
