 ...
 * Single source of truth for all calculator business logic.
 * 
 * [V3.6] UNIFIED - Added power operation support.
 *
...
import { MAX_DIGITS, MAX_VALUE, MIN_VALUE } from '../types/Calculator';
 ...
 * [V3.6] Added power operator (^) support
 * @param prev - First operand (string or number)
 * @param next - Second operand (string or number)
...
export function calculate(
  ...
  switch (operation) {
    ...
    case '^': // [V3.6] Power operation
      result = Math.pow(prevNum, nextNum);
      break;
    default:
      return null;
  }
...
export function formatNumber(value: string): string {
  ...
  if (formatted.replace('.', '').replace('-', '').length > MAX_DIGITS) {
    ...
    if (formatted.includes('.')) {
      ...
      if (intPart && intPart.replace('-', '').length >= precision) {
        formatted = num.toPrecision(precision);
      } else {
        const maxDec = precision - (intPart?.replace('-', '').length ?? 0);
...
export { MAX_DIGITS, MAX_VALUE, MIN_VALUE };
