...
function tokenize(expression: string): Token[] {
  ...
  while (i < expr.length) {
    ...
    if (char !== undefined && /[-\d.]/.test(char)) {
      ...
      if (char === '-' && (!lastToken || lastToken.type === 'operator' || lastToken.type === 'lparen')) {
        // Negative number
        i++;
        while (i < expr.length && expr[i] !== undefined && /[\d.]/.test(expr[i]!)) {
          num += expr[i];
          i++;
        }
    ...
    if (char !== undefined && /[a-zA-Z]/.test(char)) {
      let varName = '';
      while (i < expr.length && expr[i] !== undefined && /[a-zA-Z0-9]/.test(expr[i]!)) {
        varName += expr[i];
        i++;
      }
...
function normalizeOp(op: string): string {
  ...
  return operationMap[op.toLowerCase()] ?? op;
}
...
export function parseExpression(input: string, variables: Map<string, number>): ParsedExpression | null {
  ...
  if (firstToken && firstToken.type === 'number') {
    operand1 = parseFloat(firstToken.value);
  } else if (firstToken && firstToken.type === 'variable') {
    const varVal = variables.get(firstToken.value.toLowerCase());
    if (varVal === undefined) return null;
    operand1 = varVal;
  } else if (firstToken && firstToken.type === 'lparen') {
    // Handle (expr) as operand - find matching paren
    let parenDepth = 1;
    let j = i + 1;
    while (j < tokens.length && parenDepth > 0) {
      const t = tokens[j];
      if (t && t.type === 'lparen') parenDepth++;
      if (t && t.type === 'rparen') parenDepth--;
      j++;
    }
    // Simple parenthesized number - extract it
    const nextToken = tokens[i + 1];
    if (j - i === 3 && nextToken && nextToken.type === 'number') {
      operand1 = parseFloat(nextToken.value);
    }
  }
  i++;
  ...
  if (opToken && i < tokens.length && opToken.type === 'operator') {
    operator = normalizeOp(opToken.value);
    i++;
  }
  
  // Parse second operand
  const secondToken = tokens[i];
  if (secondToken && i < tokens.length && operand1 !== null) {
    if (secondToken.type === 'number') {
      operand2 = parseFloat(secondToken.value);
    } else if (secondToken.type === 'variable') {
      const varVal = variables.get(secondToken.value.toLowerCase());
      if (varVal === undefined) return null;
      operand2 = varVal;
    }
  }
...
export class REPL {
  ...
  async start(): Promise<void> {
    ...
    const isTTY = (this.inputStream as any).isTTY;
    if (this.enableKeyboard && isTTY) {
  ...
  evaluate(expression: string): CalculationResult {
    ...
    try {
      const result = calculate(op as any, parsed.operand1, parsed.operand2);
      if (result.success) {
  ...
  private handleStore(parts: string[]): void {
    ...
    if (namePart === undefined || valPart === undefined) {
      this.outputStream.write(
        color('red', 'Usage: store <name> <value>\n')
      );
      return;
    }

    const name = namePart.toLowerCase();
    const value = parseFloat(valPart);
  ...
  private addToHistory(input: string): void {
    ...
    if (lastEntry && lastEntry.expression === input) {
      return;
    }
...
export function runFromArgs(args: string[]): string | null {
  ...
  if (arg1 === undefined || op === undefined || arg2 === undefined) {
