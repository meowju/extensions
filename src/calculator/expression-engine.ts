...
export class Parser {
   ...
   * expression → term (('+' | '-') term)*
   * [V3.6] Fixed: was returning immediately instead of continuing the loop
   */
  public parseExpression(): ASTNode {
    ...
    while (this.match(TokenType.PLUS, TokenType.MINUS)) {
      ...
      node = this.createBinaryOp(operator, node, right);
    }
   ...
   * power → unary ('^' power)*  // [V3.6] Right-associative: a^b^c = a^(b^c)
   */
  private parsePower(): ASTNode {
    let left = this.parseUnary();
    
    while (this.match(TokenType.POWER)) {
      // [V3.6] Right-associative: call parsePower() instead of parseUnary()
      // to allow further right-side power operations
      const right = this.parsePower();
  ...
  private parsePrimary(): ASTNode {
    ...
    // Handle unexpected tokens (like extra closing parenthesis)
    // e.g., "5 + 3)" - extra ')' after complete expression
...
export class Evaluator {
   ...
   */
  private evaluateNumber(node: NumberNode): EvalResult<number> {
