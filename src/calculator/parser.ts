...
export class ExpressionParser {
  ...
  private toRPN(): Token[] {
    ...
    while (this.pos < this.tokens.length) {
      ...
      switch (token.type) {
        ...
        // MODULO (added in V3.6)
    case TokenType.MODULO:
      // [V3.6] Handle modulo like division - same precedence level
      // For "10 % 3", we treat it like a binary operation
      // If there's a pending unary minus, handle it first
      while (operatorStack.length > 0) {
        const top = operatorStack[operatorStack.length - 1];
        if (top.type === TokenType.LPAREN) break;
        const topPrec = this.getPrecedence(top.type);
  ...
  private evaluateRPN(rpn: Token[]): EvalResult {
    ...
    for (const token of rpn) {
      switch (token.type) {
        ...
        case TokenType.MULTIPLY:
        case TokenType.DIVIDE:
        case TokenType.MODULO:   // [V3.6] Handle modulo
        case TokenType.POWER:
          if (stack.length < 2) {
   ...
   * [V3.6] Added MODULO at same level as DIVIDE
   */
  private getPrecedence(opType: TokenType): number {
    switch (opType) {
      ...
      case TokenType.MULTIPLY:
      case TokenType.DIVIDE:
      case TokenType.MODULO:  // [V3.6] Modulo has same precedence as div
        return 2;
      case TokenType.POWER:
        return 3;
      default:
  ...
  private tokenTypeToSymbol(opType: TokenType): string {
    switch (opType) {
      ...
      case TokenType.MODULO: return '%';  // [V3.6] Added modulo
