...
import { searchMemory, storeMemory, formatSearchResults, type MemoryEntry as KernelMemoryEntry } from "../../../kernel/src/sidecars/memory-fts.ts";
...
export class SovereignPalace {
  ...
  private proprioceptiveCache: Map<string, string> = new Map();
  ...
  async memorize(
    ...
    tags: string[] = [],
    importance: number = 3,
    source: KernelMemoryEntry["source"] = "agent"
  ): Promise<string> {
  ...
  async recall(context: string, limit = 10): Promise<MemoryEntry[]> {
    ...
    return results.map(r => ({
      key: r.entry.key,
      value: r.entry.value,
      timestamp: r.entry.timestamp ? new Date(r.entry.timestamp).getTime() : Date.now(),
      tags: r.entry.tags || [],
      source: r.entry.source || "memory",
      importance: r.entry.importance || 3,
      accessCount: 0,
      lastAccessed: Date.now()
    }));
  }
  ...
  adaptParameters(
    ...
    if (state.alertness < 0.4) {
      adapted.verbose = 0; // Be concise when tired
    }
...
async function run() {
  if (command === "feel") {
    const errorsStr = args[1];
    const successStr = args[2];
    const state = await palace.feel({
      recentErrors: errorsStr ? parseInt(errorsStr) : 0,
      recentSuccesses: successStr ? parseInt(successStr) : 0,
      currentTask: args[3] || "general"
    });
