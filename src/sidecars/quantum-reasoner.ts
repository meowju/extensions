...
function parseNetworkMap(): EntanglementMatrix {
  ...
  if (matrix["Q2"]) matrix["Q2"]["Q1"] = "HI";
  if (matrix["Q3"]) matrix["Q3"]["Q1"] = "HI";
  if (matrix["Q5"] && matrix["Q5"]["Q1"]) matrix["Q5"]["Q1"] = "ME";
...
export async function evaluateHypotheses(changes: ChangeRequest[]): Promise<QuantumDecision[]> {
  const decisions = await Promise.all(changes.map(c => quantumReason(c)));
