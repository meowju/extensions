...
export function getLastSessionId(): string | null {
  // autoResume: retrieves the last session ID for session continuity
  try {
...
// EPOCH 22: Memory pressure API for lean-agent auto-compact
...
export function getSessionMemoryPressure(sessionId: string): MemoryPressureResult {
  try {
    const messages = loadSession(sessionId);
    ...
    return {
      messageCount: count,
      pressure,
      shouldCompact: count >= COMPACT_THRESHOLD,
    };
  } catch {
    return { messageCount: 0, pressure: "low", shouldCompact: false };
  }
}
