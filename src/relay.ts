...
for (let i = 0; i < cliArgs.length; i++) {
  ...
  if (arg === "--channel") {
    const val = cliArgs[++i];
    if (val) argChannels.push(val);
  } else if (arg === "--prefix") {
    const val = cliArgs[++i];
    if (val) argPrefix = val;
  } else if (arg === "--mention-only") {
    argMentionOnly = true;
  }
}
...
const RELAY_PREFIX: string = argPrefix || process.env.RELAY_PREFIX || "";
...
const DEBUG_MODE = process.env.DEBUG_DISCORD === "1";  // Debug mode: log to console instead of Discord API
 ...
 * 
 * DEBUG MODE: When DEBUG_DISCORD=1, logs to console instead of Discord API.
 * This allows testing the relay end-to-end without sending messages to Discord.
 */
const CHUNK_DELAY_MS = 100; // 100ms between chunks (Cursor pattern)

/**
 * Debug wrapper for sending messages.
 * In debug mode, logs to console instead of calling Discord API.
 */
async function debugSend(
  ...
  if (DEBUG_MODE) {
    console.log(`[DEBUG:DISCORD] → [${channelId}] ${content.slice(0, 200)}${content.length > 200 ? "..." : ""}`);
  } else {
    await replyFn();
  }
}
...
async function sendChunksWithRateLimit(message: Message, chunks: string[]): Promise<void> {
  for (let i = 0; i < chunks.length; i++) {
    ...
    if (chunk) {
      await debugSend(message.channelId, chunk, () => message.reply(chunk));
    }
...
async function executeBackupCommands(promptText: string, reply: string): Promise<BackupResult | null> {
  ...
  if (isRestoreRequest) {
    const repo = backupRepo || settings.backupRepo;
    if (!repo) {
      return { success: false, output: "No backup repo configured. Please provide your backup repo URL first." };
    }
...
class ClaudeCodeClient {
  ...
  private async promptWithRetry(text: string, attempt: number): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const execPath = "node";
      const configContent = existsSync(mcpConfig) ? readFileSync(mcpConfig, "utf-8") : "{}";
      const hasServers = configContent.includes('"mcpServers"') && !configContent.includes('"mcpServers": {}') && !configContent.includes('"mcpServers":{}');
      
      const execArgs = [cliPath, ...args.filter(a => a !== "--mcp-config" && a !== mcpConfig)];
      if (hasServers) {
        execArgs.push("--mcp-config", mcpConfig);
      }
      execArgs.push("-p", text);
...
function handleMissionCommand(prompt: string, message: Message): string | null {
  ...
  if (createMatch) {
    ...
    if (!title) return "Please provide a title for the mission.";
    const missions = loadMissions();
  ...
  if (goalMatch) {
    ...
    if (!missionTitle || !goal) return "Format: `add goal to <mission>: <goal>`";
  ...
  if (startMatch) {
    ...
    if (!idOrTitle) return "Please specify which mission to start.";
  ...
  if (prompt.toLowerCase().includes("cancel mission") || prompt.toLowerCase().includes("delete mission")) {
    ...
    const missionToRemove = missions[idx];
    if (missionToRemove) {
      missions.splice(idx, 1);
      saveMissions(missions);
      return `🗑️ Mission **${missionToRemove.title}** cancelled.`;
    }
    return `Couldn't find mission "${idOrTitle}".`;
  }
    ...
    try {
      ...
      if (RELAY_TYPING && message.channel.type === ChannelType.GuildText) {
        ...
        typingInterval = setInterval(() => {
          // Don't catch errors here - let Discord API errors be silent
          // The finally block handles cleanup when message processing completes
          (message.channel as TextChannel).sendTyping().catch(() => {/* silent */});
        }, 4000);
      }
      ...
      try {
        ...
        try {
          ...
          let friendlyMsg = "Both backends failed.";
          const is429 = meowErr.message.includes("429") || claudeErr.message.includes("429");
          
          if (is429) {
            friendlyMsg = "😿 **Rate limit reached!** The AI provider is busy or your token plan limit was hit. Please try again in a few minutes.";
          } else {
            friendlyMsg = `❌ **Meow encountered an error:**\n${meowErr.message.slice(0, 500)}\n\n(Fallback also failed: ${claudeErr.message.slice(0, 200)})`;
          }

          try {
            await message.reply(friendlyMsg);
          } catch { /* ignore */ }
          processing.delete(message.id);
          return;
        }
