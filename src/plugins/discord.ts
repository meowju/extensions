...
async function sendDiscordReply(message: Message, content: string, attachments?: string[]): Promise<void> {
  if (attachments && attachments.length > 0) {
    try {
      ...
      if (content.length <= 2000) {
        ...
        if (message.channel.isTextBased()) {
          await (message.channel as any).send({ content: content.substring(0, 2000), files });
        }
      }
      return;
    } catch (error: any) {
...
// Send reply with file attachment (for PDF, images, etc.)
async function sendDiscordReplyWithFile(
  message: Message,
  content: string,
  filePath: string,
  fileName: string
  ...
  try {
    ...
    if (content.length <= 2000) {
      ...
      // Send content in parts, file separately
      if (message.channel.isTextBased()) {
        await (message.channel as any).send({ content: content.substring(0, 2000), files: [attachment] });
      }
    }
  } catch (error: any) {
    console.error("Failed to send file attachment:", error.message);
    await message.reply(`⚠️ Failed to send file: ${error.message}`);
  }
}
